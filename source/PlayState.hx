...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'hyperfunk' | 'sugarcrush':
				defaultCamZoom = 0.73;
				curStage = 'skatepark';
				var bglights:BGSprite = new BGSprite('bglights', -651, -321, 0.2, 0.2, 'nikku');
				add(bglights);

				var clouds:BGSprite = new BGSprite('clouds', -635, -325, 0.3, 0.3, 'nikku');
				add(clouds);

				var backbuilds:BGSprite = new BGSprite('backbuilds', -586, -343, 0.5, 0.5, 'nikku');
				add(backbuilds);

				var buildings:BGSprite = new BGSprite('buildings', -633, -302, 0.7, 0.7, 'nikku');
				add(buildings);

				var wallsbg:BGSprite = new BGSprite('wallsbg', -621, -302, 0.8, 0.8, 'nikku');
				add(wallsbg);

				var parkstg:BGSprite = new BGSprite('parkstg', -609, -280, 1, 1, 'nikku');
				add(parkstg);
			default:
				defaultCamZoom = 0.9;
				curStage = 'stage';
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		senpaiEvil.frames = SUtil.getPath() + Paths.getSparrowAtlas('weeb/senpaiCrazy');
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					...
					babyArrow.frames = SUtil.getPath() + Paths.getSparrowAtlas(skin);
		...
		if(isDad) {
			switch (curStage) {
				case 'skatepark':
					...
					FlxTween.tween(FlxG.camera, {zoom: 0.71}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.quadInOut});
				default:
			...
			switch (curStage) {
				case 'skatepark':
					camFollow.x = 1001.97;
					camFollow.y = 626.12;
					FlxTween.tween(FlxG.camera, {zoom: 0.79}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.quadInOut});
				default:
					switch (curStage)
					{
						case 'limo':
